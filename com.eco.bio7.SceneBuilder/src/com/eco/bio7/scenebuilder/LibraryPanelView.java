		...
		if (editor instanceof MultiPageEditor) {
			...
			Platform.runLater(new Runnable() {
				...
				public void run() {
					...
					if (pag != null) {
						SearchController searchController= new SearchController(pag.editorController);
									
						LibraryPanelController libraryPanelController = new LibraryPanelController(pag.editorController,
								PreferencesController.getSingleton().getMavenPreferences());
						//MessageBarController m = new MessageBarController(pag.editorController);
						//libraryPanelController.performImportJarFxml(pag.editorController.getOwnerWindow());
						BorderPane pane = new BorderPane();
						
						pane.setTop(searchController.getPanelRoot());
						pane.setCenter(libraryPanelController.getPanelRoot());
						
						scene = new Scene(pane);
						
						searchController.textProperty().addListener((ov, oldStr, newStr) -> 
